/* ============================================================
   ...
   - 自動回收站會在遊玩與離線期間累積再生材料（lobbyEconomy.js）。
...
(function (global) {
  ...
  var CAMERA_VERTICAL_BIAS = 0.60;/* 角色略低於中心，保留視野給北側傳送門 */
  ...
  var GUIDE_STEPS = [
    ...
    },
    {
      visual: "⬢",
      title: "回收材料，打造大廳",
      text: "回收站會自動累積再生材料；靠近工作台可建造、擴張土地，並開放新的島嶼空間。"
    }
  ];
  ...
  var Lobby = {
    ...
    start: function () {
      ...
      var relocatedBuildings = this.reconcileFixedLayout();
      ...
      /*
       * 存檔位置若落在阻擋內，採用碰撞系統算出的最近安全點。
       * 河道等完全非法座標會由 resolveAvatarPosition 回退到出生點；
       * 靠近固定設施時則保留在設施旁，不因修正距離稍大而被傳送走。
       */
      ...
      global.LobbyEconomy.settle(Date.now());
      if (W().inIdleZone(x, y)) global.LobbyEconomy.enterZone();
      else global.LobbyEconomy.leaveZone();
      ...
      if (relocatedBuildings > 0 && this.app && this.app.showToast) {
        this.app.showToast(
          "中央廣場已更新",
          "有 " + relocatedBuildings + " 件建築與新設施重疊，已安全移回建造庫存。"
        );
      }
    ...
    /*
     * v1.1 將固定設施移入中央廣場。舊存檔若剛好在新保留區放了建築，
     * 將物件完整收納回庫存，避免碰撞卡死，也不刪除玩家已購買內容。
     */
    reconcileFixedLayout: function () {
      var lobby = global.Storage.getLobby();
      if (!lobby || !Array.isArray(lobby.buildings) || !lobby.buildings.length) return 0;
      var facilityRects = (W().reservedRects || []).filter(function (rect) {
        return rect.name !== "出生點安全區";
      });
      var kept = [];
      var relocated = 0;
      lobby.inventory = lobby.inventory || {};
      lobby.buildings.forEach(function (inst) {
        if (inst.placed === false) { kept.push(inst); return; }
        var def = global.GameData.getLobbyBuilding(inst.buildingId);
        if (!def) { kept.push(inst); return; }
        var rect = P().instanceRect(inst, def);
        var overlaps = facilityRects.some(function (reserved) {
          return rect.x < reserved.x + reserved.w && rect.x + rect.w > reserved.x &&
            rect.y < reserved.y + reserved.h && rect.y + rect.h > reserved.y;
        });
        if (!overlaps) { kept.push(inst); return; }
        lobby.inventory[inst.buildingId] = (lobby.inventory[inst.buildingId] | 0) + 1;
        relocated += 1;
      });
      if (!relocated) return 0;
      lobby.buildings = kept;
      global.Storage.save();
      return relocated;
    },
    ...
    update: function (dt) {
      ...
      if (this.mode === "idle") {
        ...
        if (mv.x || mv.y) {
          ...
          av.bobT += dt;
          if (global.Tutorial) {
            var moved = Math.sqrt(Math.pow(av.x - prevX, 2) + Math.pow(av.y - prevY, 2));
            global.Tutorial.notify("lobby-moved", { distance: moved });
          }
        }
    ...
    updateInteractions: function () {
      var av = this.avatar;
      var recycle = W().idleZone;
      var recycleStatus = global.LobbyEconomy.getStatus();
      var candidates = [
        { kind: "portal", def: W().portal, label: "選擇行動地點", icon: "▶" },
        { kind: "workbench", def: W().workbench, label: "開啟建造工作台", icon: "🔨" },
        {
          kind: "recycle",
          def: { x: recycle.x + recycle.w / 2, y: recycle.y + recycle.h / 2, interactRadius: Math.max(recycle.w, recycle.h) * 0.72 },
          label: recycleStatus.unclaimed > 0 ? "領取 " + recycleStatus.unclaimed + " 份再生材料" : "查看自動回收站",
          icon: "♻"
        }
      ];
      var expansionLevel = global.Storage.getLobbyExpansionLevel ? global.Storage.getLobbyExpansionLevel() : 0;
      Object.keys(W().facilities || {}).forEach(function (id) {
        var facility = W().facilities[id];
        if ((facility.unlockLevel | 0) > expansionLevel) return;
        candidates.push({ kind: "facility:" + id, def: facility, label: "進入" + facility.label, icon: facility.glyph || "⌂" });
      });
      var nearest = null;
    ...
    triggerInteraction: function () {
      ...
      else if (this.nearestInteraction.kind === "recycle") {
        var amount = global.LobbyEconomy.collect();
        if (amount > 0) {
          this.addFloater(this.avatar.x, this.avatar.y - 40, "+" + amount + " 再生材料", "#8fd06a");
          this.updateHud();
          if (global.Tutorial) global.Tutorial.notify("recycle-collected", { amount: amount });
          if (global.UI) global.UI.showToast("回收完成", "已領取 " + amount + " 份再生材料；回收站會繼續自動累積。");
        } else if (global.UI) {
          global.UI.showToast("回收站仍在運作", "目前沒有待領材料，稍後再回來看看。");
        }
        this.updateInteractions();
      }
      else if (this.nearestInteraction.kind.indexOf("facility:") === 0 && this.app && this.app.openIslandSpace) {
        this.app.openIslandSpace(this.nearestInteraction.kind.split(":")[1]);
      }
    },
    ...
    updateObjective: function (nearKind) {
      ...
      if (nearKind === "portal") {
        ...
        this.dom.objectiveText.textContent = touch
          ? "工作台可用：點互動提示開始建造"
          : "工作台可用：按 E 開啟建造目錄";
      } else if (nearKind === "recycle") {
        var recycleStatus = global.LobbyEconomy.getStatus();
        this.dom.objectiveText.textContent = recycleStatus.unclaimed > 0
          ? "自動回收已備妥：領取 " + recycleStatus.unclaimed + " 份再生材料"
          : "自動回收運作中：不必留在這裡等待";
      } else if (nearKind && nearKind.indexOf("facility:") === 0) {
        var facilityId = nearKind.split(":")[1];
        var facility = W().facilities && W().facilities[facilityId];
        this.dom.objectiveText.textContent = (facility ? facility.label : "島嶼空間") + "已抵達：" + (touch ? "點互動提示進入" : "按 E 進入");
      } else {
    ...
    showGuideIfNeeded: function () {
      if (global.Tutorial) {
        this.hideGuide();
        if (global.MobileFit && global.MobileFit.shouldShowRotateHint && global.MobileFit.shouldShowRotateHint()) {
          this._guidePending = true;
          return;
        }
        global.Tutorial.startIfNeeded();
        return;
      }
    ...
    updateIdleEconomy: function () {
      ...
      var sig = [inZone, status.earned, status.unclaimed, status.secondsToNext, status.capped].join("|");
    ...
    updateIdleStatus: function (status) {
      ...
      var interval = global.LobbyEconomy.getInterval ? global.LobbyEconomy.getInterval() : (global.LobbyEconomy.IDLE_INTERVAL || 20);
      ...
      if (box) {
        if (!status.inZone) {
          ...
          box.innerHTML = status.unclaimed > 0
            ? "<strong>可領取 " + formatNumber(status.unclaimed) + " 份</strong><span>按 E 或點互動提示收菜</span>"
            : status.capped
              ? "<strong>今日回收已完成</strong><span>明日會重新開始生產</span>"
              : "<strong>自動回收運作中</strong><span>下一份材料：約 " + seconds + " 秒</span>" +
                "<span class='idle-progress-track' aria-hidden='true'><i style='width:" + progress.toFixed(1) + "%'></i></span>";
        }
      }

      if (this.dom.daily) {
        this.dom.daily.textContent = status.unclaimed > 0
          ? "回收站可領 " + formatNumber(status.unclaimed) + " · 今日 " + formatNumber(status.earned) + " / " + formatNumber(status.cap)
          : status.capped
            ? "今日回收已滿 " + formatNumber(status.earned) + " / " + formatNumber(status.cap)
            : "自動累積中 · 下一份約 " + seconds + " 秒";
      }
      if (this.dom.buildRecycleProgress) {
        ...
        this.dom.buildRecycleProgress.textContent = status.unclaimed > 0
          ? "回收站待領：" + status.unclaimed + " 份"
          : status.capped
            ? "今日自動回收已達上限"
    ...
    buildCatalog: function () {
      ...
      list.classList.toggle("land-mode", tab === "land");
      list.innerHTML = "";
      if (tab === "land" && global.IslandSpaces && global.IslandSpaces.renderLandCatalog) {
        global.IslandSpaces.renderLandCatalog(list);
        return;
      }
    ...
    openLandCatalog: function () {
      this._catalogTab = "land";
      if (this.mode !== "catalog") this.enterBuild();
      else this.buildCatalog();
    },
    ...
    confirmGhost: function () {
      ...
      if (global.Tutorial) global.Tutorial.notify("building-placed", { building: result.building || null });
      this.ghost = null;
      this.mode = "catalog";
      if (this.dom.ghostBar) this.dom.ghostBar.classList.add("hidden");
    ...
    isWorldRectVisible: function (rect, padding) {
      padding = Math.max(0, Number(padding) || 0);
      var z = W().ZOOM;
      var vw = this.canvas.width / z;
      var vh = this.canvas.height / z;
      return rect.x + rect.w >= this.camera.x - padding &&
        rect.x <= this.camera.x + vw + padding &&
        rect.y + rect.h >= this.camera.y - padding &&
        rect.y <= this.camera.y + vh + padding;
    },
    ...
    updateCamera: function (snap, dt) {
      ...
      var tx = target.x - vw / 2, ty = target.y - vh * CAMERA_VERTICAL_BIAS;
    ...
    render: function () {
      ...
      this.drawExpansionLand(ctx);
      if (collisionDebugEnabled()) this.drawCollisionDebug(ctx);

      this.drawIdleZone(ctx);
      this.drawWorkbench(ctx);
      this.drawPortal(ctx);
      this.drawIslandFacilities(ctx);

      /* 建築與角色以 y 排序繪製（腳底基準），維持前後遮擋關係 */
      var drawables = [];
      var self = this;
      (W().ambientDecorations || []).forEach(function (ambient, index) {
        var def = global.GameData.getLobbyBuilding(ambient.id);
        if (!def) return;
        var size = P().footprintSize(def, ambient.rotation || 0);
        var rect = { x: ambient.x, y: ambient.y, w: size.w * W().CELL, h: size.h * W().CELL };
        drawables.push({
          baseY: rect.y + rect.h,
          draw: function () {
            self.drawBuilding(ctx, {
              instanceId: "ambient-" + index,
              buildingId: ambient.id,
    ...
    drawExpansionLand: function (ctx) {
      var level = global.Storage.getLobbyExpansionLevel ? global.Storage.getLobbyExpansionLevel() : 0;
      var locked = (W().walkablePolygons || []).filter(function (poly) { return (poly.unlockLevel | 0) > level; });
      if (!locked.length) return;
      ctx.save();
      locked.forEach(function (poly) {
        if (!poly.points || !poly.points.length) return;
        ctx.beginPath();
        ctx.moveTo(poly.points[0][0], poly.points[0][1]);
        for (var i = 1; i < poly.points.length; i++) ctx.lineTo(poly.points[i][0], poly.points[i][1]);
        ctx.closePath();
        ctx.fillStyle = "rgba(4,18,24,.68)";
        ctx.fill();
        ctx.strokeStyle = "rgba(255,212,92,.58)";
        ctx.lineWidth = 3;
    ...
    drawIslandFacilities: function (ctx) {
      var level = global.Storage.getLobbyExpansionLevel ? global.Storage.getLobbyExpansionLevel() : 0;
      var self = this;
      Object.keys(W().facilities || {}).forEach(function (id) {
        var facility = W().facilities[id];
        if ((facility.unlockLevel | 0) > level) return;
        var near = self.nearestInteraction && self.nearestInteraction.kind === "facility:" + id;
        var colors = id === "cottage" ? ["#9b6f43", "#e2be75"] : id === "laboratory" ? ["#326b79", "#78ded0"] : ["#326844", "#8fd06a"];
        ctx.save();
        ctx.fillStyle = "rgba(3,17,23,.48)";
        ctx.beginPath(); ctx.ellipse(facility.x, facility.y + 29, 48, 15, 0, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = colors[0];
        ctx.strokeStyle = near ? "#ffd45c" : colors[1];
        ctx.lineWidth = near ? 4 : 2;
        if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(facility.x - 38, facility.y - 38, 76, 66, 12); ctx.fill(); ctx.stroke(); }
        else { ctx.fillRect(facility.x - 38, facility.y - 38, 76, 66); ctx.strokeRect(facility.x - 38, facility.y - 38, 76, 66); }
        ctx.fillStyle = colors[1];
        ctx.font = "bold 34px sans-serif";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(facility.glyph || "⌂", facility.x, facility.y - 5);
        if (near) {
          ctx.strokeStyle = "rgba(255,212,92," + (0.55 + Math.sin(self.time * 5) * 0.28) + ")";
          ctx.lineWidth = 3; ctx.setLineDash([8, 6]);
          ctx.beginPath(); ctx.ellipse(facility.x, facility.y + 30, 57, 20, 0, 0, Math.PI * 2); ctx.stroke();
        }
        ctx.restore();
      });
    },
    ...
    /*
     * 設施名稱使用真正的世界座標；鏡頭移動不再把文字夾在螢幕邊緣。
     * 設施完全離開視野時連同標籤一起隱藏，導航另由任務提示負責。
     */
    drawStationLabels: function (ctx) {
      if (!global.UI_THEME) return;
      var portal = W().portal;
      var workbench = W().workbench;
      var zone = W().idleZone;
      var recycleX = zone.x + zone.w / 2;
      if (this.isWorldRectVisible({ x: portal.x - 64, y: portal.y - 80, w: 128, h: 178 }, 4)) {
        global.UI_THEME.drawOutlinedText(ctx, "行動傳送門", portal.x, portal.y + 98, {
          fontSize: 16, fill: "#c9f5ec", strokeWidth: 3
        });
      }
      if (this.isWorldRectVisible({ x: workbench.x - 58, y: workbench.y - 64, w: 116, h: 112 }, 4)) {
        global.UI_THEME.drawOutlinedText(ctx, "建造工作台", workbench.x, workbench.y - 58, {
          fontSize: 16, fill: "#ffe1b3", strokeWidth: 3
        });
      }
      if (this.isWorldRectVisible({ x: zone.x, y: zone.y, w: zone.w, h: zone.h }, 4)) {
        global.UI_THEME.drawOutlinedText(ctx, "資源回收區", recycleX, zone.y - 12, {
          fontSize: 16, fill: "#bdf4ec", strokeWidth: 3
        });
      }
      var level = global.Storage.getLobbyExpansionLevel ? global.Storage.getLobbyExpansionLevel() : 0;
      Object.keys(W().facilities || {}).forEach(function (id) {
        var facility = W().facilities[id];
        if ((facility.unlockLevel | 0) > level) return;
        if (this.isWorldRectVisible({ x: facility.x - 70, y: facility.y - 70, w: 140, h: 145 }, 4)) {
          global.UI_THEME.drawOutlinedText(ctx, facility.label, facility.x, facility.y + 68, { fontSize: 15, fill: "#fff0ad", strokeWidth: 3 });
        }
      }, this);
    },
