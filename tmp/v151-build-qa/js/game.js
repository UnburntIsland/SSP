...
(function (global) {
  ...
  var Game = {
    ...
    abort: function (options) {
      ...
      this.bossDefeated = false; this.bossSpawned = false; this.bossSpawnReason = null;
    ...
    update: function (dt) {
      ...
      /*
       * 先處理單次事件，再判斷延長賽。即使分頁恢復造成時間一次跳過
       * BOSS 預定點，事件仍會先補發；若關卡資料缺少事件，
       * updateOvertime() 也會在計時歸零時做最後保證。
       */
      this.handleEvents();
    ...
    spawnOne: function (enemyId, forceBoss) {
      ...
      if (def.isBoss && (!this.stage.bossId || def.id === this.stage.bossId)) {
        ...
        if (!this.bossSpawnReason) this.bossSpawnReason = forceBoss ? "event" : "runtime";
    ...
    /*
     * 關卡 BOSS 的單一入口。時間事件、計時歸零保證與測試工具都可重複
     * 呼叫，但同一局永遠只建立一隻指定 BOSS。
     */
    ensureBossSpawned: function (reason) {
      if (!this.stage || !this.stage.bossId || this.ended || this.bossDefeated) return null;
      if (this.bossSpawned) {
        for (var i = 0; i < this.enemies.length; i++) {
          var existing = this.enemies[i];
          if (!existing.dead && existing.isBoss && existing.id === this.stage.bossId) return existing;
        }
        /* bossSpawned 是一次性閘門；即使外部程式誤移除實例，也不複製 BOSS。 */
        return null;
      }
      var def = global.GameData.getEnemy(this.stage.bossId);
      if (!def || !def.isBoss) return null;
      this.bossSpawnReason = reason || "guarantee";
      var boss = this.spawnOne(def.id, true);
      if (!boss) this.bossSpawnReason = null;
      return boss;
    },
    ...
    handleEvents: function () {
      ...
      for (var i = 0; i < evs.length; i++) {
        ...
        if (!this.firedEvents[i] && this.time >= ev.at * (this.timelineScale || 1)) {
          ...
          var isStageBoss = !!this.stage.bossId && ev.enemy === this.stage.bossId;
          var spawned = null;
          if (isStageBoss) {
            spawned = this.ensureBossSpawned("event");
          } else {
            for (var c = 0; c < (ev.count || 1); c++) {
              spawned = this.spawnOne(ev.enemy, true) || spawned;
            }
          }
          if (spawned && ev.announce && this.app) this.app.showToast("警報", ev.announce);
        }
      }
    },
    ...
    applyQuizAnswer: function (result) {
      if (!result) return;
      if (global.Storage && global.Storage.recordQuestionAttempt && result.question) {
        global.Storage.recordQuestionAttempt(result.question, result.selected, result.correct === true);
      }
      if (global.Tutorial) global.Tutorial.notify("quiz-answered", result);
