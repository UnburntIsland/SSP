...
(function (global) {
  ...
  var SCREENS = {
    ...
    islandSpaces: "screen-island-spaces",
  ...
  var App = {
    ...
    boot: function () {
      ...
      if (global.TestMode && global.TestMode.enabled && global.TestMode.lobbyMaterials > 0) {
        var qaLobby = global.Storage.getLobby();
        qaLobby.materials.recycled = Math.max(qaLobby.materials.recycled | 0, global.TestMode.lobbyMaterials | 0);
        global.Storage.save({ source: "qa-materials" });
      }
      ...
      if (global.Tutorial) global.Tutorial.init(this);
      ...
      this.enterLobby();
      if (global.CloudSync) global.CloudSync.init(this);
      if (global.Education) global.Education.init(this);
      if (global.IslandSpaces) global.IslandSpaces.init(this);
    ...
    openPortalSelect: function () {
      ...
      if (global.Tutorial) global.Tutorial.notify("portal-opened");
    ...
    setState: function (s) {
      this.state = s;
      if (global.Tutorial) global.Tutorial.onStateChange();
    },
    ...
    bindKeys: function () {
      ...
      global.addEventListener("keydown", function (e) {
        ...
        if (c === "Escape" && self.ui.isQuestionReviewVisible && self.ui.isQuestionReviewVisible()) {
          e.preventDefault();
          self.ui.hideQuestionReview();
          return;
        }
        if (c === "Escape" && self.ui.isCloudConflictVisible && self.ui.isCloudConflictVisible()) {
          e.preventDefault();
          self.ui.hideCloudConflict();
          return;
        }
        ...
        if (st === "PLAYING") { e.preventDefault(); self.pause(); }
          ...
          else if (st === "ISLAND_SPACES") {
            e.preventDefault();
            if (!global.IslandSpaces || !global.IslandSpaces.back()) self.handleAction("back");
          }
          else if (st === "CHARACTER_SELECT" || st === "GACHA" || st === "SHOP" || st === "ACHIEVEMENTS" || st === "ENVIRONMENT_MISSIONS" || st === "CODEX" || st === "HELP" || st === "ACCOUNT" || st === "EDUCATION") { e.preventDefault(); self.handleAction("back"); }
        }
      });
    },
    ...
    handleAction: function (action, source) {
      ...
      switch (action) {
        ...
        case "review-question-open": this.ui.showQuestionReview(source && source.dataset.questionId); break;
        case "review-question-close": this.ui.hideQuestionReview(); break;
        case "help":          this.showScreen("help"); this.setState("HELP"); break;
        case "account":       this.openAccount(); break;
        case "education":     this.openEducation(); break;
        case "education-refresh": if (global.Education) global.Education.refresh(); break;
        case "education-open-account": this.enterLobby(); this.openAccount(); break;
        case "education-join-class": if (global.Education && this.ui.getEducationJoinCode) global.Education.joinClass(this.ui.getEducationJoinCode()); break;
        case "education-create-class": if (global.Education && this.ui.getEducationClassName) global.Education.createClass(this.ui.getEducationClassName()); break;
        case "education-select-class": if (global.Education) global.Education.selectClass(source && source.dataset.classId); break;
        case "education-create-assignment": if (global.Education && this.ui.getEducationAssignmentDraft) global.Education.createAssignment(this.ui.getEducationAssignmentDraft()); break;
        case "education-review-accept": if (global.Education) global.Education.review(source && source.dataset.assignmentId, source && source.dataset.studentId, "accepted", ""); break;
        case "education-review-return": {
          var reviewRow = source && source.closest ? source.closest(".teacher-review-row") : null;
          var reviewFeedback = reviewRow && reviewRow.querySelector(".teacher-review-feedback");
          if (global.Education) global.Education.review(source && source.dataset.assignmentId, source && source.dataset.studentId, "needs_revision", reviewFeedback && reviewFeedback.value);
          break;
        }
        case "island-spaces": this.openIslandSpaces(); break;
        case "island-open-space": this.openIslandSpace(source && source.dataset.spaceId); break;
        case "island-space-back": if (!global.IslandSpaces || !global.IslandSpaces.back()) this.enterLobby(); break;
        case "island-go-build": this.enterLobby(); this.enterBuildMode(); if (global.Lobby) global.Lobby.openLandCatalog(); break;
        case "island-expand": if (global.IslandSpaces) global.IslandSpaces.expand(source && source.dataset.expansionLevel); break;
        case "cottage-rest": if (global.IslandSpaces) global.IslandSpaces.restCottage(); break;
        case "laboratory-research": if (global.IslandSpaces) global.IslandSpaces.research(source && source.dataset.researchId); break;
        case "recycle-yard-collect": if (global.IslandSpaces) global.IslandSpaces.collectRecycle(); break;
        case "recycle-game-start": if (global.IslandSpaces) global.IslandSpaces.startRecycleGame(); break;
        case "recycle-sort-choice": if (global.IslandSpaces) global.IslandSpaces.sortChoice(source && source.dataset.binId); break;
        case "recycle-sort-next": if (global.IslandSpaces) global.IslandSpaces.nextSortItem(); break;
        case "recycle-game-dismiss": if (global.IslandSpaces) global.IslandSpaces.dismissGame(); break;
        case "cloud-login":   this.cloudPasswordAction("signin"); break;
        case "cloud-register": this.cloudPasswordAction("signup"); break;
        case "cloud-google":  if (global.CloudSync) global.CloudSync.google(false); break;
        case "cloud-link-google": if (global.CloudSync) global.CloudSync.google(true); break;
        case "cloud-sync":    if (global.CloudSync) global.CloudSync.syncNow({ manual: true }); break;
        case "cloud-autosync": if (global.CloudSync) global.CloudSync.toggleAutoSync(); break;
        case "cloud-logout":  if (global.CloudSync) global.CloudSync.signOut(); break;
        case "cloud-conflict-local": if (global.CloudSync) global.CloudSync.resolveConflict("local"); break;
        case "cloud-conflict-cloud": if (global.CloudSync) global.CloudSync.resolveConflict("cloud"); break;
        case "cloud-conflict-later": if (this.ui.hideCloudConflict) this.ui.hideCloudConflict(); break;
    ...
    openAccount: function () {
      ...
      if (this.ui.renderAccount) this.ui.renderAccount(global.CloudSync ? global.CloudSync.getState() : {});
    },

    openEducation: function () {
      if (this.state !== "LOBBY") return;
      this.showScreen("education");
      this.setState("EDUCATION");
      if (this.ui.renderEducation) this.ui.renderEducation(global.Education ? global.Education.getState() : {});
      if (global.Education && global.CloudSync && global.CloudSync.user) global.Education.refresh();
    },

    openIslandSpaces: function () {
      if (this.state !== "LOBBY") return;
      this.showScreen("islandSpaces");
      this.setState("ISLAND_SPACES");
      if (global.IslandSpaces) {
        global.IslandSpaces.message = "";
        global.IslandSpaces.renderHub();
      }
    },

    openIslandSpace: function (spaceId) {
      if (!global.IslandSpaces || !global.IslandSpaces.isUnlocked(spaceId)) {
        if (global.UI) global.UI.showToast("空間尚未開放", "請先到建造工作台完成對應的土地擴張。");
        return;
      }
      if (this.state === "LOBBY") {
        this.showScreen("islandSpaces");
        this.setState("ISLAND_SPACES");
      } else if (this.state !== "ISLAND_SPACES") return;
      global.Storage.visitIslandSpace(spaceId);
      global.IslandSpaces.message = "";
      global.IslandSpaces.renderSpace(spaceId);
    },

    cloudPasswordAction: function (kind) {
      if (!global.CloudSync || !this.ui.getCloudCredentials) return;
      var credentials = this.ui.getCloudCredentials();
      if (kind === "signup") global.CloudSync.signUp(credentials.email, credentials.password);
      else global.CloudSync.signIn(credentials.email, credentials.password);
    },

    reloadFromCloudSave: function () {
      this.selectedCharacterId = this.loadSelectedCharacter();
      this.currentSelectedCharacter = this.selectedCharacterId;
      this.candidateCharacterId = this.selectedCharacterId;
      this.selectedChar = this.selectedCharacterId;
      this.currentChar = this.selectedCharacterId;
      this.selectedStageId = this.loadSelectedStage();
      this.currentStageId = this.selectedStageId;
      this.ui.updateHomeCharacterPreview(this.selectedCharacterId);
      ...
      this.updateStageSelector();
      this.enterLobby();
      if (this.ui.showToast) this.ui.showToast("雲端進度已載入", "角色、關卡、資源與答題紀錄都已更新。");
    },
    ...
    resetSave: function () {
      var startingCoins = global.Storage && global.Storage.getStartingCoins ? global.Storage.getStartingCoins() : 200;
      this.ui.setConfirm(
        "確定重置所有存檔？",
        "循環幣會恢復為 " + startingCoins.toLocaleString("zh-TW") + "；角色與造型、技能點、永久升級、關卡進度與圖鑑都會重置。此操作無法復原。",
        "確認重置"
      );
    ...
    startRun: function (charId, stageId, challengeId) {
      ...
      this.setState("PLAYING");
      if (global.Tutorial) global.Tutorial.notify("stage-started");
    },
