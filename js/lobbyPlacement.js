...
(function (global) {
  ...
  var LobbyWorld = {
    ...
    facilities: {
      cottage: { x: 470, y: 228, interactRadius: 86, unlockLevel: 1, label: "林間小屋", glyph: "⌂" },
      laboratory: { x: 1125, y: 244, interactRadius: 86, unlockLevel: 2, label: "生態實驗室", glyph: "⚗" },
    ...
    ambientDecorations: [
      ...
      { id: "sorting_bins", x: 1030, y: 575, rotation: 0 }
    ],
    ...
    walkablePolygons: [
      ...
      },
      {
        ...
        points: [
          ...
          [1165, 235], [1180, 275], [1150, 315], [1115, 315],
          [1070, 280], [1060, 245], [1070, 195], [1080, 160]
        ]
      },
      {
    ...
    reservedRects: [
      ...
      { x: 1055, y: 186, w: 145, h: 112, name: "生態實驗室入口" },
      { x: 1235, y: 420, w: 180, h: 145, name: "分類回收場入口" }
    ],
    ...
    reachTargets: function () {
      var targets = [
        ...
        { x: this.workbench.x + 76, y: this.workbench.y, name: "工作台" },
        { x: this.idleZone.x + 20, y: this.idleZone.y + this.idleZone.h / 2, name: "掛機回收區" }
      ];
      var level = this.getExpansionLevel();
      if (level >= 1) targets.push({ x: this.facilities.cottage.x + 68, y: this.facilities.cottage.y + 28, name: "林間小屋" });
      if (level >= 2) {
        targets.push({ x: this.facilities.laboratory.x, y: this.facilities.laboratory.y, name: "生態實驗室" });
        targets.push({ x: this.facilities.recycleYard.x - 72, y: this.facilities.recycleYard.y + 35, name: "分類回收場" });
      }
      return targets;
    },

    getExpansionLevel: function () {
      return global.Storage && global.Storage.getLobbyExpansionLevel
        ? global.Storage.getLobbyExpansionLevel()
        : 0;
    },
    isTierUnlocked: function (level) { return this.getExpansionLevel() >= Math.max(0, level | 0); },
    ...
    pointInWalkable: function (x, y) {
      ...
      var onLand = this.walkablePolygons.some(function (poly) {
        if ((poly.unlockLevel | 0) > level) return false;
    ...
    staticBlocked: function (cx, cy) {
      ...
      var level = this.getExpansionLevel();
      if (!this._staticGrid || this._staticGridLevel !== level) this._buildStaticGrid();
      return this._staticGrid[cy * COLS + cx];
    },
    _buildStaticGrid: function () {
      this._staticGridLevel = this.getExpansionLevel();
