  ...
  var UI = {
    init: function (app) {
      ...
      this.dom = {
        ...
        questionReviewOverlay: $("overlay-question-review"),
        questionReviewTitle: $("question-review-title"),
        questionReviewBody: $("question-review-body"),
        accountContent: $("account-content"),
        cloudAccountButton: $("cloud-account-button"),
        cloudAccountStatus: $("cloud-account-status"),
        cloudConflictOverlay: $("overlay-cloud-conflict"),
        cloudConflictLocal: $("cloud-conflict-local"),
        cloudConflictCloud: $("cloud-conflict-cloud"),
        educationContent: $("education-content"),
        educationMenuButton: $("education-menu-button"),
        educationMenuLabel: $("education-menu-label"),
        educationMenuBadge: $("education-menu-badge"),
        hud: $("hud"),
    ...
    updateCloudBadge: function (state) {
      if (!this.dom) return;
      state = state || {};
      var labels = {
        unavailable: "本機存檔", starting: "檢查中", guest: "訪客",
        authenticating: "登入中", ready: "已登入", syncing: "同步中",
        synced: "已同步", offline: "離線", error: "需檢查", conflict: "選擇存檔"
      };
      var label = labels[state.status] || (state.signedIn ? "已登入" : "訪客");
      if (this.dom.cloudAccountStatus) this.dom.cloudAccountStatus.textContent = label;
      if (this.dom.cloudAccountButton) {
        this.dom.cloudAccountButton.dataset.cloudStatus = state.status || "starting";
        this.dom.cloudAccountButton.classList.toggle("has-pending", !!state.pending || state.status === "conflict");
        this.dom.cloudAccountButton.setAttribute("aria-label", "帳號與雲端存檔，目前狀態：" + label);
      }
    },

    getCloudCredentials: function () {
      var email = $("cloud-email");
      var password = $("cloud-password");
      return { email: email ? email.value : "", password: password ? password.value : "" };
    },

    renderAccount: function (state) {
      if (!this.dom || !this.dom.accountContent) return;
      state = state || (global.CloudSync && global.CloudSync.getState ? global.CloudSync.getState() : {});
      this.updateCloudBadge(state);
      var root = this.dom.accountContent;
      var busy = state.status === "authenticating" || state.syncing;
      var messageClass = state.status === "error" || state.status === "offline" || state.status === "conflict" ? " warning" : "";
      var message = '<div class="account-status-message' + messageClass + '" role="status">' + escapeHtml(state.message || "") + '</div>';
        ...
        root.innerHTML =
          '<div class="account-grid guest">' +
            ...
            '<form class="account-panel account-form" onsubmit="return false" autocomplete="on">' +
              ...
              '<input id="cloud-email" name="email" type="email" inputmode="email" autocomplete="email" maxlength="254" placeholder="player@example.com" ' + (busy ? "disabled" : "") + ' />' +
              '<label for="cloud-password">密碼 <small>至少 8 個字元</small></label>' +
              '<input id="cloud-password" name="password" type="password" autocomplete="current-password" minlength="8" maxlength="128" placeholder="輸入密碼" ' + (busy ? "disabled" : "") + ' />' +
              '<div class="account-form-actions"><button class="btn btn-primary" type="button" data-action="cloud-login" ' + (busy ? "disabled" : "") + '>登入</button>' +
              '<button class="btn" type="button" data-action="cloud-register" ' + (busy ? "disabled" : "") + '>建立帳號</button></div>' +
              '<div class="account-divider"><span>或</span></div>' +
              '<button class="btn google-button" type="button" data-action="cloud-google" ' + (busy ? "disabled" : "") + '><span class="google-mark" aria-hidden="true">G</span>使用 Google 帳號</button>' +
              '<p class="account-privacy">帳號驗證由雲端服務處理；遊戲不會儲存你的密碼。</p>' +
            '</form>' +
          '</div>' + message;
        this.hideCloudConflict();
        return;
      ...
      var user = state.user || {};
      var providers = (user.identities || []).map(function (identity) { return identity && identity.provider; }).filter(Boolean);
      ...
      var providerLabels = providers.map(function (provider) {
        return '<span class="account-provider">' + escapeHtml(provider === "email" ? "Email" : provider === "google" ? "Google" : provider) + '</span>';
      }).join("") || '<span class="account-provider">Email</span>';
      root.innerHTML =
        '<div class="account-grid signed">' +
          '<article class="account-panel account-profile"><div class="account-avatar" aria-hidden="true">守</div><div>' +
          ...
          '</article>' +
          '<article class="account-panel account-security"><div><h3>登入方式</h3><p>可以將 Google 連結到目前帳號，之後使用同一身分登入。</p></div>' +
            (hasGoogle ? '<span class="linked-google"><span class="google-mark" aria-hidden="true">G</span>Google 已連結</span>' : '<button class="btn google-button" type="button" data-action="cloud-link-google" ' + (busy ? "disabled" : "") + '><span class="google-mark" aria-hidden="true">G</span>連結 Google</button>') +
          '</article>' +
          '<article class="account-panel account-signout"><div><h3>離開這個帳號</h3><p>登出不會刪除本機或雲端進度。</p></div><button class="btn" type="button" data-action="cloud-logout" ' + (busy ? "disabled" : "") + '>登出</button></article>' +
        '</div>' + message;
      if (state.conflict) this.showCloudConflict(state);
      else this.hideCloudConflict();
    },

    showCloudConflict: function (state) {
      if (!this.dom || !this.dom.cloudConflictOverlay || !state || !state.conflict) return;
      ...
      if (this.dom.cloudConflictOverlay.classList.contains("hidden")) openDialog(this.dom.cloudConflictOverlay, '[data-action="cloud-conflict-cloud"]');
    },

    hideCloudConflict: function () {
      if (this.dom && this.dom.cloudConflictOverlay) closeDialog(this.dom.cloudConflictOverlay);
    },

    isCloudConflictVisible: function () {
      return !!(this.dom && this.dom.cloudConflictOverlay && !this.dom.cloudConflictOverlay.classList.contains("hidden"));
    },

    updateEducationBadge: function (state) {
      if (!this.dom) return;
      state = state || {};
      var label = state.role === "teacher" ? "教師後台" : state.role === "student" ? "我的班級" : "班級";
      if (this.dom.educationMenuLabel) this.dom.educationMenuLabel.textContent = label;
      var count = 0;
      if (state.role === "teacher") count = (state.submissions || []).filter(function (row) { return row.status === "pending_review"; }).length;
      else if (state.role === "student") {
        var accepted = {};
        (state.submissions || []).forEach(function (row) { if (row.status === "accepted") accepted[row.assignment_id] = true; });
        count = (state.assignments || []).filter(function (item) { return !accepted[item.id]; }).length;
      }
      if (this.dom.educationMenuBadge) {
        this.dom.educationMenuBadge.textContent = String(Math.min(99, count));
        this.dom.educationMenuBadge.classList.toggle("hidden", count <= 0);
      }
      if (this.dom.educationMenuButton) {
        this.dom.educationMenuButton.setAttribute("aria-label", label + (count ? "，" + count + " 項待處理" : ""));
      }
    },

    getEducationJoinCode: function () {
      var node = $("education-class-code");
      return node ? node.value : "";
    },

    getEducationClassName: function () {
      var node = $("teacher-class-name");
      return node ? node.value : "";
    },

    getEducationAssignmentDraft: function () {
      function value(id) { var node = $(id); return node ? node.value : ""; }
      return {
        title: value("teacher-assignment-title"),
        description: value("teacher-assignment-description"),
        kind: value("teacher-assignment-kind"),
        stageId: value("teacher-assignment-stage"),
        count: Number(value("teacher-assignment-count")) || 1,
        dueAt: value("teacher-assignment-due")
      };
    },

    renderEducation: function (state) {
      if (!this.dom || !this.dom.educationContent) return;
      state = state || {};
      this.updateEducationBadge(state);
      var root = this.dom.educationContent;
      ...
      if (state.role === "teacher") this.renderTeacherDashboard(state, root, message);
      else this.renderStudentClassroom(state, root, message);
    },

    renderClassTabs: function (state) {
      if (!state.classes || !state.classes.length) return "";
      return '<div class="education-class-tabs" role="tablist" aria-label="選擇班級">' + state.classes.map(function (item) {
        var active = item.id === state.selectedClassId;
        return '<button type="button" role="tab" aria-selected="' + (active ? "true" : "false") + '" class="education-class-tab' + (active ? " active" : "") + '" data-action="education-select-class" data-class-id="' + escapeHtml(item.id) + '">' + escapeHtml(item.name) + '</button>';
      }).join("") + '</div>';
    },

    renderStudentClassroom: function (state, root, message) {
      var selected = (state.classes || []).find(function (item) { return item.id === state.selectedClassId; }) || null;
      var submissions = {};
      (state.submissions || []).forEach(function (row) { submissions[row.assignment_id] = row; });
      var statusLabel = { in_progress: "進行中", pending_review: "等待老師驗收", accepted: "老師已驗收", needs_revision: "老師退回重做" };
      var cards = selected ? (state.assignments || []).filter(function (item) { return item.class_id === selected.id; }).map(function (assignment) {
        var row = submissions[assignment.id] || null;
        var metric = assignment.metric || { progress: 0, goal: 1, met: false, label: "完成學習任務" };
        var progress = Math.max(metric.progress, row && row.progress || 0);
        var percent = Math.max(0, Math.min(100, Math.round(progress / Math.max(1, metric.goal) * 100)));
        var status = row ? row.status : "in_progress";
        var due = assignment.due_at ? new Date(assignment.due_at).toLocaleString("zh-TW", { hour12: false }) : "沒有期限";
        return '<article class="student-assignment-card status-' + escapeHtml(status) + '"><div class="student-assignment-head"><div><span class="assignment-status">' + escapeHtml(statusLabel[status] || "進行中") + '</span><h3>' + escapeHtml(assignment.title) + '</h3></div><span class="assignment-due">' + escapeHtml(due) + '</span></div>' +
          '<p>' + escapeHtml(assignment.description || metric.label) + '</p><strong class="assignment-goal">' + escapeHtml(metric.label) + '</strong>' +
          '<div class="assignment-progress" role="progressbar" aria-valuemin="0" aria-valuemax="' + metric.goal + '" aria-valuenow="' + Math.min(progress, metric.goal) + '"><span style="width:' + percent + '%"></span></div>' +
          '<div class="assignment-progress-label"><span>' + formatNumber(progress) + ' / ' + formatNumber(metric.goal) + '</span><span>' + percent + '%</span></div>' +
          (row && row.feedback ? '<div class="assignment-feedback">老師回饋：' + escapeHtml(row.feedback) + '</div>' : '') + '</article>';
      }).join("") : "";
      root.innerHTML = '<div class="student-classroom">' +
        '<section class="education-toolbar"><div><span class="education-role-chip student">學生</span><strong>' + escapeHtml(state.profile.display_name || "守護者") + '</strong></div>' +
    ...
    renderTeacherDashboard: function (state, root, message) {
      var selected = (state.classes || []).find(function (item) { return item.id === state.selectedClassId; }) || null;
      var members = selected ? (state.members || []).filter(function (item) { return item.class_id === selected.id; }) : [];
      var assignments = selected ? (state.assignments || []).filter(function (item) { return item.class_id === selected.id; }) : [];
      var profiles = {};
      (state.profiles || []).forEach(function (profile) { if (profile) profiles[profile.user_id] = profile; });
      var submissions = (state.submissions || []).filter(function (row) { return assignments.some(function (item) { return item.id === row.assignment_id; }); });
      var pending = submissions.filter(function (row) { return row.status === "pending_review"; }).length;
      var accepted = submissions.filter(function (row) { return row.status === "accepted"; }).length;
      var stageOptions = (global.GameData && global.GameData.stages || []).map(function (stage) { return '<option value="' + escapeHtml(stage.id) + '">' + escapeHtml(stage.name) + '</option>'; }).join("");
      var assignmentCards = assignments.map(function (assignment) {
        var rows = submissions.filter(function (row) { return row.assignment_id === assignment.id; });
        var wait = rows.filter(function (row) { return row.status === "pending_review"; });
        var pass = rows.filter(function (row) { return row.status === "accepted"; }).length;
        var reviewRows = wait.map(function (row) {
          var profile = profiles[row.student_id] || {};
          var source = row.evidence && row.evidence.source;
          var trust = source === "server_event_log" || source === "server_event_log_qa" ? "事件紀錄已同步" : "含舊版匯入紀錄";
          return '<div class="teacher-review-row"><div class="teacher-review-student"><strong>' + escapeHtml(profile.display_name || "學生") + '</strong><span>進度 ' + formatNumber(row.progress) + ' · ' + escapeHtml(cloudDate(row.completed_at)) + '</span><small>' + trust + '</small></div><label class="teacher-review-feedback-label">退回回饋（選填）<textarea class="teacher-review-feedback" maxlength="300" rows="2" placeholder="例如：請先完成錯題訂正再送出"></textarea></label><div class="teacher-review-actions"><button class="btn btn-primary" type="button" data-action="education-review-accept" data-assignment-id="' + escapeHtml(row.assignment_id) + '" data-student-id="' + escapeHtml(row.student_id) + '">驗收通過</button><button class="btn" type="button" data-action="education-review-return" data-assignment-id="' + escapeHtml(row.assignment_id) + '" data-student-id="' + escapeHtml(row.student_id) + '">退回重做</button></div></div>';
        }).join("");
        return '<article class="teacher-assignment-card"><div class="teacher-assignment-head"><div><span class="education-role-chip assignment">作業</span><h3>' + escapeHtml(assignment.title) + '</h3></div><div class="teacher-assignment-counts"><span>待驗 ' + wait.length + '</span><span>通過 ' + pass + '</span></div></div><p>' + escapeHtml(assignment.description || "沒有補充說明") + '</p>' +
          (reviewRows ? '<div class="teacher-review-list">' + reviewRows + '</div>' : '<div class="teacher-review-empty">目前沒有等待驗收的成果</div>') + '</article>';
      }).join("");
      var matrixHead = assignments.map(function (item) { return '<th>' + escapeHtml(item.title) + '</th>'; }).join("");
      var matrixRows = members.map(function (member) {
        var profile = profiles[member.student_id] || {};
        var cells = assignments.map(function (assignment) {
          var row = submissions.find(function (item) { return item.assignment_id === assignment.id && item.student_id === member.student_id; });
          var text = !row ? "未開始" : row.status === "accepted" ? "已通過" : row.status === "pending_review" ? "待驗收" : row.status === "needs_revision" ? "需重做" : "進行中";
          return '<td class="matrix-' + escapeHtml(row && row.status || "none") + '">' + text + '</td>';
        }).join("");
        return '<tr><th>' + escapeHtml(profile.display_name || "學生") + '</th>' + cells + '</tr>';
      }).join("");
      root.innerHTML = '<div class="teacher-dashboard">' +
    ...
    setCodexView: function (view) {
      if (["enemies", "knowledge", "questions"].indexOf(view) === -1) view = "enemies";
    ...
    buildCodex: function () {
      ...
      if (!list) return;
      this.updateAchievementMenuBadge();
      var view = ["knowledge", "questions"].indexOf(this._codexView) !== -1 ? this._codexView : "enemies";
      list.innerHTML = "";
      list.className = "codex-list " + (view === "knowledge" ? "codex-knowledge-list" : (view === "questions" ? "codex-question-review" : "codex-enemy-list"));
      list.setAttribute("aria-labelledby", view === "knowledge" ? "codex-tab-knowledge" : (view === "questions" ? "codex-tab-questions" : "codex-tab-enemies"));
      if (this.dom.codexDesc) {
        this.dom.codexDesc.textContent = view === "knowledge"
          ? "在關卡中拾取知識卡，逐步解鎖永續行動與環境知識。"
          : view === "questions"
            ? "查看答對率、完成率與訂正狀態；所有題目都能再次作答。"
            : "查看各關卡出現的污染物、威脅類型與行動提示。";
      }
      if (view === "knowledge") this.buildKnowledgeCodex(list);
      else if (view === "questions") this.buildQuestionCodex(list);
      else this.buildEnemyCodex(list);
    },

    buildQuestionCodex: function (list) {
      var self = this;
      var summary = global.Storage.getQuestionSummary();
      var stats = el("div", "question-review-summary");
      [
        [summary.completionRate + "%", "完成率"],
        [summary.accuracyRate + "%", "作答正確率"],
        [String(summary.wrong), "待訂正"],
        [String(summary.corrected), "已訂正"]
      ].forEach(function (entry) {
        var card = el("div", "question-review-stat");
        card.appendChild(el("strong", "", entry[0]));
        card.appendChild(el("span", "", entry[1]));
        stats.appendChild(card);
      });
      list.appendChild(stats);

      var filter = this._questionFilter || "all";
      var filters = el("div", "question-review-filters");
      [
        ["all", "全部"], ["correct", "答對"], ["wrong", "答錯"],
        ["corrected", "已訂正"], ["unanswered", "未作答"]
      ].forEach(function (entry) {
        var button = el("button", "question-review-filter" + (filter === entry[0] ? " active" : ""), entry[1]);
        button.type = "button";
        button.setAttribute("aria-pressed", String(filter === entry[0]));
        button.addEventListener("click", function () {
          self._questionFilter = entry[0];
          self.buildCodex();
        });
      ...
      var questionList = el("div", "codex-question-list");
      var questions = global.GameData.sustainabilityQuestions || [];
      var visible = questions.filter(function (question) {
        return filter === "all" || global.Storage.getQuestionStatus(question.id) === filter;
      });
      var labels = { correct: "答對", wrong: "答錯", corrected: "已訂正", unanswered: "未作答" };
      var symbols = { correct: "✓", wrong: "!", corrected: "↻", unanswered: "?" };
      visible.forEach(function (question) {
        var status = global.Storage.getQuestionStatus(question.id);
        var progress = global.Storage.getQuestionProgress(question.id);
        var card = el("article", "codex-question-card status-" + status);
        card.appendChild(el("span", "question-status-dot", symbols[status]));
        var copy = el("div", "question-card-copy");
        copy.appendChild(el("strong", "", question.prompt));
        copy.appendChild(el("span", "", labels[status] + (progress ? " · 已作答 " + progress.attempts + " 次" : "")));
        card.appendChild(copy);
        var open = el("button", "btn question-card-open", status === "wrong" ? "立即訂正" : "重新作答");
        open.type = "button";
        open.dataset.action = "review-question-open";
        open.dataset.questionId = question.id;
        card.appendChild(open);
        questionList.appendChild(card);
      });
      if (!visible.length) questionList.appendChild(el("p", "achievement-empty", "這個分類目前沒有題目。"));
      list.appendChild(questionList);
      if (global.Tutorial) global.Tutorial.notify("review-opened");
    },

    showQuestionReview: function (questionId) {
      var questions = global.GameData.sustainabilityQuestions || [];
      var question = questions.find(function (item) { return item.id === questionId; });
      if (!question || !this.dom.questionReviewBody) return;
      var body = this.dom.questionReviewBody;
      var self = this;
      var answered = false;
      if (this.dom.questionReviewTitle) this.dom.questionReviewTitle.textContent = question.prompt;
      body.innerHTML = "";
      var options = el("div", "question-practice-options");
      question.options.forEach(function (answer, index) {
        var button = el("button", "btn question-practice-option", (index + 1) + ". " + answer);
        button.type = "button";
        button.addEventListener("click", function () {
          if (answered) return;
          answered = true;
          var correct = index === question.answer;
          global.Storage.recordQuestionAttempt(question, index, correct);
          Array.prototype.forEach.call(options.children, function (node, optionIndex) {
            node.disabled = true;
            if (optionIndex === question.answer) node.classList.add("correct");
            else if (optionIndex === index) node.classList.add("wrong");
          });
          var detail = el("div", "question-detail-box");
          detail.appendChild(el("strong", "", correct ? "答對了！" : "這次答錯了，已記入待訂正。"));
          detail.appendChild(el("div", "question-detail-answer", "正確答案：" + question.options[question.answer]));
          detail.appendChild(el("div", "", "詳解：" + question.explanation));
          var close = el("button", "btn btn-primary", "完成複習");
          close.type = "button";
          close.addEventListener("click", function () { self.hideQuestionReview(); });
      ...
      openDialog(this.dom.questionReviewOverlay, ".question-practice-option");
    },

    hideQuestionReview: function () {
      closeDialog(this.dom.questionReviewOverlay);
    },

    isQuestionReviewVisible: function () {
      return !!(this.dom.questionReviewOverlay && !this.dom.questionReviewOverlay.classList.contains("hidden"));
    },
    ...
    showSustainabilityQuiz: function (question, onAnswer) {
      ...
      function choose(index) {
        ...
        feedback.appendChild(el("div", "quiz-result", correct
          ? "答對了：回復生命、獲得 ♻2，並累積連勝"
          : "答錯了：承受少量污染壓力，連勝重新計算"));
        var explanation = el("div", "quiz-explanation question-detail-box");
        explanation.appendChild(el("strong", "question-detail-answer", "正確答案：" + question.options[question.answer]));
        explanation.appendChild(el("span", "", "詳解：" + question.explanation));
        explanation.appendChild(el("span", "", "這次結果會保存到「圖鑑／成就 → 題目複習」，答錯後可再次作答完成訂正。"));
